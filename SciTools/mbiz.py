...
from mutil import Cfg, Utils, PandasUtil
...
class CleanBiz:
    ...
    @staticmethod
    def repalce_values(
            df:pd.DataFrame, names:List[str], current_tab_index:int, old_sep:str, new_sep:str, other_char:str, is_reserved:bool, is_new:bool
    ):
        """
        替换值
        :param df:
        :param names: 列名
        :param current_tab_index: 哪种方式，0还是1
    ...
    @staticmethod
    def combine_synonym(
            df: pd.DataFrame, words_dict: Dict[str, str], names: List[str], is_new: bool
    ) -> pd.DataFrame:
        """
        合并词
        :param df:  数据集
...
class AnalyzeBiz:
    ...
    def count(df:pd.DataFrame, by:str) -> Dict[str, int]:
        ...
        :param df:
        :param by: 一个分组字段
        :return: 返回字典，key是by，value是count
        """
        ...
        result = count_result.set_index(by)['Count'].to_dict()
        return result
