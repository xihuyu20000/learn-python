...
class AuthorWordsCouplingAnalysis:
    ...
    """
    def __init__(self, dataset: pd.DataFrame, author_col_name: str, words_col_name: str, author_freq_threshold: int = 0, words_freq_threshold: int = 0):
    ...
    def stat_authors_freq(self):
        ...
        # 保留高产作者
        self.reserved_author_count_pairs:Dict[str,int] = {author: times for author, times in author_count.items() if times >= self.author_freq_threshold}
    ...
    def stat_words_freq(self):
        ...
        # 保留高频词
        self.reserved_words_count_pairs:Dict[str,int] = {word: times for word, times in words_count.items() if times >= self.words_freq_threshold}
    ...
    def stat_each_author_each_word_freq(self):
        ...
        self.author_word_freq_matrix = self.author_word_thin_matrix.pivot_table(index='author', columns='word', values='times')
...
if __name__ == '__main__':
    ...
    cc = AuthorWordsCouplingAnalysis(df, author_col_name='A1', words_col_name='K1', author_freq_threshold=0, words_freq_threshold=0)
    cc.run()
